...
import { Loader2, ArrowLeft, Mail, Sparkles, Shield, Users, Star, Award, CheckCircle, TrendingUp, Zap } from 'lucide-react';
...
export default function ForgotPassword({ status: initialStatus }: { status?: string }) {
    ...
    useEffect(() => {
        ...
        let rafq: number;
        const updateRect = () => {
            rect = document.getElementById('forgot-form')?.getBoundingClientRect();
        };
        updateRect();
    ...
    const submit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await axios.post('/api/password/send-code', { email });
            setStatus(response.data.message || 'Password reset code sent! Please check your email.');
        } catch (error: any) {
    ...
    return (
        <div className="min-h-screen flex flex-col lg:flex-row bg-gradient-to-br from-slate-50 via-white to-orange-50/30 font-sans selection:bg-orange-200 relative">
            <Head title="Forgot Password - Hirfati" />
            ...
            <div className="w-full lg:w-1/2 flex flex-col justify-center p-6 sm:p-12 lg:p-16 xl:p-24 relative z-10 bg-white min-h-screen">
                ...
                >
                    ...
                    >
                        ...
                        <div className="relative my-8">
                            ...
                            <div className="relative flex justify-center text-sm">
                                <span className="px-4 bg-white text-slate-500 font-semibold">Remembered your password?</span>
                            </div>
                        </div>
                    ...
                    >
                        {[
                            { icon: Shield, label: 'Secure Recovery' },
                            { icon: CheckCircle, label: 'Safe Process' }
                        ].map((item, idx) => (
                            <motion.div
                                key={idx}
                    ...
                    >
                        <div className="mb-10">
                            ...
                            >
                                ...
                                <span className="text-orange-200 text-sm font-bold uppercase tracking-wider">Account Security</span>
                            ...
                            <motion.p
                                ...
                                transition={{ delay: 0.8 }}
                            >
                                Your security is our top priority. Reset your password quickly and get back to managing your services.
                            </motion.p>
                        </div>

                        {/* Enhanced Benefit Card with 3D */}
                        <motion.div
                        ...
                        >
                            ...
                            <div className="relative bg-white/10 backdrop-blur-xl p-6 rounded-3xl border border-white/20 shadow-2xl">
                                <div className="flex items-start gap-4">
                                    <motion.div
                                        className="w-14 h-14 rounded-2xl bg-gradient-to-br from-green-400 to-green-600 flex items-center justify-center text-white flex-shrink-0 shadow-xl"
                                    ...
                                    <div>
                                        <p className="text-white/95 text-sm leading-relaxed mb-3 font-medium">
                                            "I forgot my password but was able to reset it in seconds. The process is so seamless and secure!"
                                        </p>
                                        <div className="flex items-center gap-2">
                                            <span className="text-white font-bold text-sm">Verified User</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        >
                            {[
                                { icon: Shield, label: 'Data Protected', color: 'from-blue-400 to-blue-600' },
                                { icon: Award, label: '256-bit Encryption', color: 'from-purple-400 to-purple-600' }
                            ].map((badge, idx) => (
                                <motion.div
                                    key={idx}
                                    whileHover={{ y: -3, scale: 1.05 }}
