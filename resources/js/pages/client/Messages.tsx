...
const normalizeConversation = (raw: any): Conversation => ({
    ...
    contact: {
        ...
        avatar: resolveAvatarUrl(raw?.contact?.avatar ?? null),
    },
    lastMessage:
        raw?.lastMessage === null || raw?.lastMessage === undefined
            ? null
            : String(raw.lastMessage),
...
export default function ClientMessages() {
    ...
    // Parse order ID from URL on initial load
    const initialOrderId = useMemo(() => {
        if (typeof window !== 'undefined') {
            const params = new URLSearchParams(window.location.search);
            const orderParam = params.get('order');
            if (orderParam) {
                const parsed = Number.parseInt(orderParam, 10);
    ...
    const reloadThread = async (orderId: number) => {
        const res = await axios.get(`/api/client/messages/${orderId}`, {
            headers: authHeaders,
        });
        const rawMessages =
        ...
        const nextContact = res.data?.data?.contact ?? null;
        setContact(
            nextContact
                ? {
                      ...nextContact,
                      avatar: resolveAvatarUrl(nextContact.avatar),
                  }
                : null,
        );
    ...
    useEffect(() => {
        const load = async () => {
            ...
            setError(null);
            try {
                const res = await axios.get('/api/client/messages', {
                    headers: authHeaders,
                });
                const items =
                ...
                if (selectedOrderId) {
                    ...
                    if (!exists) {
                        setSelectedOrderId(normalized[0]?.orderId ?? null);
                    }
                } else if (normalized.length > 0) {
                    setSelectedOrderId(normalized[0].orderId);
                }
            } catch {
    ...
    const handleSend = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const res = await axios.post(
                `/api/client/messages/${selectedOrderId}`,
                formData,
                {
                    headers: {
                        ...authHeaders,
